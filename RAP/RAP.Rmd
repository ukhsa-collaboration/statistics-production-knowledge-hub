...
versioncontrol: "`r ukhsaGuidanceDocs::VersionHistory( c('10/11/2022', 'Third draft'), c('03/11/2022', 'Second draft'),  c('10/10/2022', 'First draft')  )`"
---
...
This guidance applies to any projects involving the analysis of numerical information. 
...
1. Iterative improvements should be made to analytical pipelines to work towards our [Bronze, Silver and Gold standards of RAP](...).
...
The aim for UKHSA, and across Government, is to work towards "RAP by default" for all analysis `r addRef(35, 41)`. A cultural change is needed to achieve this. The purpose of this guidance is to help drive this change, outlining what RAP is, and how to achieve it. 
...
Legacy analytical pipelines tend to look like the one presented in Figure 1. Here, data is manually extracted from some central database, which might be done by another team, and saved on a local or network drive as a CSV file. Proprietary software such as SPSS, Stata and or Excel is then used to process that data, relying on manual "point-and-click" operations. Charts and tables are separately produced, which are then copied and pasted into a word document. Numbers reported within the text of the report is manually updated. That document is emailed for quality assurance and returned for revisions. The word document is then converted to markdown (or saved as a pdf), ready for publication. 
...
* where there isn't full documentation of the analytical process, there is a reliance on analysts remembering which series of steps they should take to produce an output, meaning that when those analysts leave the organisation or go on leave, it may be difficult for others to reproduce those steps
...
We have developed [Bronze, Silver and Gold standards of RAP](...) to provide major benchmarks for analytical teams to work towards, based upon the "Minimum" and "further" standards of RAP developed by the Government Analysis Function `r addRef(18)`. 
...
New tools and skills may be needed to implement RAP. Senior leaders should acknowledge and support this, and analysts should make efforts to pursue them too `r addRef(18, 31)`. 
