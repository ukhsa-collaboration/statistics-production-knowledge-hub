...
This guidance applies to all external releases of microdata, defined as record-level data, where, for example, each row defines an individual, individual household or employer, for example `r addRef(15)`. 
...
The Responsible Statistician should ensure that the following steps are undertaken for all releases:

1. Prior to any release of microdata, a risk assessment should be conducted to determine the likelihood and impact of any disclosure risks. K-anonymity and intruder testing are methods that can be used for these purposes. 
...
Many of the same risk factors given in our guidance on [statistical disclosure control for aggregate data](https://htmlpreview.github.io/?https://github.com/UKHSA-Statistics-Production/knowledge-hub/blob/main/Disclosure%20control/Disclosure-control-for-aggregate-data.html) will still apply here. 
...
* the presence of direct identifiers (for example, name, postcode, NHS number) - in most cases it will be crucial that these are identified and removed prior to data release `r addRef(15)`
