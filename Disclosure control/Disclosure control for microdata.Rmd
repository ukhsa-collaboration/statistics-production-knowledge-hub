...
a. Any direct identifiers (for example, name, NHS number) should always be removed from the data as these will clearly identify an individual^`r addRef(15)`,`r addRef(12)`^.[^note1] 
...
a. Where there are individual 'combinations at risk' (in other words, where a data subject might be the only one with a certain set of characteristics), suppression could be used to hide an individual value, replacing it with 'c' (for confidential) or similar^`r addRef(13)`,`r addRef(16)`^. Note that it is important to apply this in such a way that the reason for suppression, and therefore the underlying value, is not made obvious ^`r addRef(13)`^. 
